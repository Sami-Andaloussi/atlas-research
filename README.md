...
Atlas Research is an active build. This repository is the public window on it: the design decisions
behind it, what runs today, and what does not work yet. The implementation is in a private
repository.
...
The constraints below were chosen deliberately, ahead of the implementation.
...
refuses. This constraint came first; the rest was built around it.

**Point-in-time is a first-class axis.** Economic data gets revised: GDP for 2024 is not the same
number in March and in September. Most systems overwrite. This store keeps *what was known, and
when*, so a past statement stays checkable.

**Public sources only.** No paid data broker. This narrows what the product can promise — Swiss
unlisted companies do not publish accounts, so Atlas will never claim to have them — and it means
any source used here can be re-fetched by a reader.

**A classification layer sits above the data.** Research on a country and research on a company
should be readable the same way. Three planes are kept separate: what exists, the question being
asked, and the operational data behind it. A sector is not a company; an instrument is not its
underlying.

**Validation can refuse the person building the system.** The build stops when a claim is not
...
tooling commits when the product itself has not advanced. That last one has refused work here.

**LLM agents are orchestrated as an engineering method.** Several agents run in parallel on separate
branches and are merged through a written inspection procedure. The rule that matters most: an
agent never reviews its own work — every fix goes to a separate agent that did not write it.
Disagreements are arbitrated by a human, who owns the decision. Details in
...
The defect count is published on purpose. It stayed frozen for 24 days while a lot of other work
happened, which was the signal that the work was not going where it was supposed to.
